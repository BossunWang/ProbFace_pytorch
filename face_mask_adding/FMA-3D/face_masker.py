...
class FaceMasker:
    ...
    def add_mask_one(self, image_path, face_lms, template_name, masked_face_path, is_68_landmarks):
        ...
        if c == 4:
            image = image[:,:,:3]
        pos, vertices = self.get_vertices(face_lms, image, is_68_landmarks) #3d reconstruction -> get texture.
        image = image/255. #!!
    ...
    def add_mask_from_img(self, image, face_lms, template_name, is_68_landmarks=False):
        """Add mask to one image.
        ...
        tmp = new_image * face_mask[:, :, np.newaxis]
        new_image = image * (1 - face_mask[:, :, np.newaxis]) + new_image * face_mask[:, :, np.newaxis]
        new_image = np.clip(new_image, 0, 1)  # must clip to (-1, 1)!
        new_image = (new_image * 255.0).astype(np.uint8)
        return new_image

    def get_vertices(self, face_lms, image, is_68_landmarks=False):
        """Get vertices

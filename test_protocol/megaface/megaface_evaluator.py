...
class MegaFaceEvaluator(metaclass=ABCMeta):
    ...
    def get_megaface_feature_list(self):
        ...
        for megaface_file_path in tqdm(self.megaface_path_list):
    ...
    def get_facescrub_feature_list(self):
        ...
        for facescrub_path in tqdm(self.facescrub_path_list):
            facescrub_path = facescrub_path.replace('.png', '.npy')
    ...
    def test_cmc(self, model_name, max_rank):
        ...
        """
        pretty_tabel = PrettyTable(["model name", "rank", "accuracy", "G_mean", "I_mean", "FDR"])
        for cur_rank in range(max_rank):
            cur_rank += 1
            cur_accuracy, FDR, positive_score_mean, negative_score_mean = self.get_rankN_accuracy(cur_rank)
            logger.info('Rank %d accuracy: %f.' % (cur_rank, cur_accuracy))
            pretty_tabel.add_row([model_name, cur_rank, cur_accuracy, positive_score_mean, negative_score_mean, FDR])
        print(pretty_tabel)
        csv_file_name = 'megaface'
...
class MaskedMegaFaceEvaluator(MegaFaceEvaluator):
    ...
    Attributes:
        ...
        masked_croped_facescrub_folder(str): masked images origin dir for copy wrong images
        masked_facescrub_wrong_image_dir(str): masked images dir for check wrong images
    ...
    def __init__(self, facescrub_json_list, megaface_json_list,
                 ...
                 masked_facescrub_feature_dir, masked_croped_facescrub_folder, masked_facescrub_wrong_image_dir,
                 is_concat):
        """Init CommonMegaFaceEvaluator by some initial files. 
        ...
        self.masked_croped_facescrub_folder = masked_croped_facescrub_folder
        self.masked_facescrub_wrong_image_dir = masked_facescrub_wrong_image_dir
        ...
        self.masked_facescrub_id2image_path_list = self.get_masked_facescrub_id2image_path_list()
        ...
        for facescrub_id, masked_feature_list in self.masked_facescrub_id2feature_list.items():
            ...
            if feature_list.shape[-1] == 513 and masked_feature_list.shape[-1] == 513:
                cur_masked_facescrub_score = MLS_score_matrix(feature_list[:, :512], masked_feature_list[:, :512]
                                                              , feature_list[:, -1], masked_feature_list[:, -1])
            else:
        ...
        if self.masked_facescrub_feature_list.shape[-1] == 513 and self.megaface_feature_list.shape[-1] == 513:
            cur_masked_facescrub_megaface_score \
                = MLS_score_matrix(self.masked_facescrub_feature_list[:, :512], self.megaface_feature_list[:, :512]
                                   , self.masked_facescrub_feature_list[:, -1], self.megaface_feature_list[:, -1])
        else:
            cur_masked_facescrub_megaface_score = \
    ...
    def get_masked_facescrub_id2image_path_list(self):
        """Get the feature list of each id in masked facescrub.

        Returns:
            A dict，key is the id in masked facescrub，value is the feature list of this id.
        """
        masked_facescrub_id2image_path_list = {}
        for facescrub_id, facescrub_path in zip(self.facescrub_id_list, self.facescrub_path_list):
            if not facescrub_id in masked_facescrub_id2image_path_list:
                facescrub_path = facescrub_path.replace('.png', '.jpg')
                facescrub_file = os.path.join(self.masked_croped_facescrub_folder, facescrub_path)
                masked_facescrub_id2image_path_list[facescrub_id] = []
            masked_facescrub_id2image_path_list[facescrub_id].append(facescrub_file)
        return masked_facescrub_id2image_path_list
            ...
            for image_index in range(M):  # Add Mi to megaface and test other M-1 photos
                for facescrub_index in range(M):
                    ...
                    if cur_positive_score >= cur_megaface_rankN_score:
                        ...
                        wrong_image_path = os.path.join(self.masked_facescrub_wrong_image_dir,
                                                        self.masked_facescrub_id2image_path_list[facescrub_id][
                                                            facescrub_index].split('/')[-1])
                        copyfile(self.masked_facescrub_id2image_path_list[facescrub_id][facescrub_index],
                                 wrong_image_path)
                    count_all += 1
                    genuine_score_list.append(cur_positive_score)
                    imposter_score_list.append(cur_megaface_rankN_score)

        accuracy = TP / count_all
        # claculate FDR
        positive_score_mean = np.mean(genuine_score_list)
        negative_score_mean = np.mean(imposter_score_list)
        positive_score_std = np.std(genuine_score_list)
        negative_score_std = np.std(imposter_score_list)

        FDR = ((positive_score_mean - negative_score_mean) ** 2) / (positive_score_std ** 2 + negative_score_std ** 2)
        return accuracy, FDR, positive_score_mean, negative_score_mean

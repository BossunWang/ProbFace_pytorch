...
def get_image_feature(img_path, img_list_path, model_type, model_path, gpu_ids, input_size, tta):
    img_list = open(img_list_path)
    embedding = Embedding(model_type, model_path, gpu_ids, input_size)
    ...
    for img_index, each_line in enumerate(files):
        ...
        lmk = lmk.reshape((5, 2))
        img_feats.append(embedding.get(img, lmk, tta))
...
if __name__ == "__main__":
    ...
    # general
    parser.add_argument('--model_type', type=str, help='', default='IR_101')
    parser.add_argument('--model_path', type=str, help='', default='')
    parser.add_argument('--gpus', type=str, help='', default='0')
    parser.add_argument('--image_size', type=str, help='', default='112,112')
    parser.add_argument('--job', default='insightface', type=str, help='job name')
    parser.add_argument('--target', default='IJBC', type=str, help='target, set to IJBC or IJBB')
    parser.add_argument('--tta', action='store_true', default=False, help='')
    args = parser.parse_args()
    target = args.target
    model_path = args.model_path
    model_type = args.model_type
    gpu_ids = [int(x) for x in args.gpus.split(',')]  # specify your GPU ids
    input_size = [int(x) for x in args.image_size.split(',')]
    tta = args.tta
    ...
    img_feats, faceness_scores = get_image_feature(img_path, img_list_path, model_type, model_path, gpu_ids, input_size,
                                                   tta)
    ...
    img_input_feats = img_feats[:, 0:int(img_feats.shape[1])]

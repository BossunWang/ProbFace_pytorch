...
def get_image_feature(img_path, files_list, model_path, model_type, gpu_id):
    batch_size = args.batch_size
    ...
    embedding = Embedding(model_path, model_type, data_shape, batch_size, gpu_id)
    ...
    embedding = Embedding(model_path, model_type, data_shape, rare_size, gpu_id)
    for img_index, each_line in enumerate(files[len(files)-rare_size:]):
        name_lmk_score = each_line.strip().split(' ')
        img_name = os.path.join(img_path, name_lmk_score[0])
        img = cv2.imread(img_name)
...
img_feats, faceness_scores = get_image_feature(img_path, files_list, model_path, model_type, gpu_id)
stop = timeit.default_timer()
print('Time: %.2f s. ' % (stop - start))
